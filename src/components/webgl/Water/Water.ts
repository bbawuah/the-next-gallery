...
const Water = function (geometry, options) {
  ...
  const mirrorShader = {
    ...
    fragmentShader: [
      ...
      '	vec3 scatter = max( 0.8, dot( surfaceNormal, eyeDirection ) ) * waterColor;',
      '	vec3 albedo = mix( ( sunColor * diffuseLight * 0.7 + scatter ) * getShadowMask(), ( vec3( 0.5 ) + reflectionSample * 0.9 + reflectionSample * specularLight ), reflectance);',
